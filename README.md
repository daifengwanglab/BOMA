...
## Summary
3D organoids cultured from induced human pluripotent stem cells (hiPSCs) are useful models for understanding the longitude genomical dynamics during human brain development, however, the genomic differences between brains and organoids are not well established. Here, we applied manifold alignment (MA) to comparatively analyze both the bulk and single-cell transcriptomic measurements over time among human brains, human organoids, and Non-Human Primate (NHP) organoids within published literature. Our MA method discovered previously known and unknown developmental patterns across the system in a much simpler manner than previous results. More importantly, the developmental trajectories generated by MA served as references for comparing single-cell transcriptomes between human brains and organoids, which allowed us to generate a database summarizing gene expression dynamics by cell-type over time. Some of the discrepancies observed between the two systems were experimentally validated as a check of method efficacy. Finally, we leverage our method to align single-cell transcriptomic measurements of human brains (n=57) and organoids (n=35) by integrating data from multiple recent publications. These alignment results demonstrate the capability of our method to integratively and comparatively analyze datasets generated from various experimental sources, which is useful for understanding the differences between brains and organoids, as well as guiding future organoid protocol design.

## Flow chart
In the flowchart below, we introduce the comparative analysis pipeline for aligning human and organoid developmental transcriptomic datasets. Manifold alignment mainly consists of two steps: global alignment and local alignment.
...
## Online Web App
An online web app to perform alignment with user provided RNA-seq and correspondence matrices can be found [here](https://github.com/Oafish1/ManiNetCluster-Visualization).


## DEMO

### Step 1: data preprocessing
We perform manifold alignment on bulk transcriptomic data of human brain from BrainSpan and the bulk transcriptomic measurement of organoids in Dataset 5(Table S1).

BrainSpan is a landscape measurement database of the developmental procedures of the human brain, concerning in-vivo human brain development from 8PCW until 40 years old, including 28 time points, 36 samples, and 25 regions. Dataset 5 is a measurement of longer-term cultured organoids, concerning in vitro 3D culturing of human cortical spheroid (hCS) from 25 days up until 2 years, including 12 time points and 62 samples.
...
```
Then, we use the limma package to perform differential analysis at each time step to find differentially expressed genes.
```R
...
To focus our analysis within brain development only, we attempt to identify 1533 genes most related to human brain development. We take the intersection of genes that are differentially expressed in human and organoid cells, then take the intersection with these 1533 genes to get the genes we are interested in.
```R
...
```
### Step 3: Alignment
ManiNetCluster employs manifold learning to uncover and match local and non-linear structures among networks, and identifies cross-network functional links. ManiNetCluster simultaneously aligns and clusters co-expression.
```R
...
Use the aligned result to draw 3D scatter plots for human and organoid, visualizing the time information of each point in color.
```R
...
Embedding of human brain samples in the aligned manifold space. Dots are colored by brain developmental stages.
```R
# plot 3D trajectors
pdf('3D1.pdf')
...
Embedding of human organoid samples in the manifold space. Dots are colored by the days of culturing. 
```R
...
Finally, draw the corrplot timepoint-wise with averaged similarity. The outcome shows the similarity of aligned samples. 
```R
library(corrplot)
